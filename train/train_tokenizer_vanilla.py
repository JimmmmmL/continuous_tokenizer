    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            with torch.amp.autocast("cuda", dtype=ptdtype):  
                recons_imgs = vq_model(imgs)
                loss_gen = F.mse_loss(recons_imgs.contiguous(), imgs.contiguous())  # Reconstruction loss
                if train_steps % args.log_every == 0:
                    logger.info(f"Step {train_steps}: Generator Loss: {loss_gen.item():.4f}")
                # log to wandb
                if rank == 0  and wandb_logger is not None:
                    wandb_logger.log({"rec_loss": loss_gen.item()}, step=train_steps)
