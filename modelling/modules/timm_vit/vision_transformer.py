...
class RoPEAttention(Attention):
    ...
    def forward(self, x, attn_mask=None):
        ...
        dtype = x.dtype
        with torch.cuda.amp.autocast(enabled=False):
            q[:, :, self.num_prefix_tokens:-self.num_latent_tokens], k[:, :, self.num_prefix_tokens:-self.num_latent_tokens] = apply_rotary_emb(q[:, :, self.num_prefix_tokens:-self.num_latent_tokens], k[:, :, self.num_prefix_tokens:-self.num_latent_tokens], freqs_cis=freqs_cis)        
            q[:, :, -self.num_latent_tokens:], k[:, :, -self.num_latent_tokens:] = apply_rotary_emb(q[:, :, -self.num_latent_tokens:], k[:, :, -self.num_latent_tokens:], freqs_cis=self.freqs_1d)
